...
if [[ "$ARCH" == "arm64" ]]; then
    ...
    sed -i 's|^LB_BOOTAPPEND_LIVE=.*|LB_BOOTAPPEND_LIVE="boot=live components username=live nomodeset clk_ignore_unused pd_ignore_unused earlycon=efifb console=tty0"|' config/binary
    sed -i 's|^LB_BOOTAPPEND_LIVE_FAILSAFE=.*|LB_BOOTAPPEND_LIVE_FAILSAFE="boot=live components nomodeset nosplash clk_ignore_unused pd_ignore_unused earlycon=efifb console=tty0"|' config/binary
